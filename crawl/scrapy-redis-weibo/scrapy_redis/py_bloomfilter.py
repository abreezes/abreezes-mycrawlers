...
import math
import time
from weibo.settings import REDIS_HOST,REDIS_PORT,REDIS_PASSWORD
...
class PyBloomFilter():
    ...
    #conn表示redis的连接客户端
    #key表示在redis中的键的名字前缀
    def __init__(self, capacity=100000000, error_rate=0.00000001,conn=None,key='BloomFilter'):
...
pool = redis.ConnectionPool(host='35.241.105.253',port=6379,password='jiange',db=2)
conn = redis.StrictRedis(connection_pool=pool)
if __name__ == '__main__':
    pass
    # conn = redis.Redis(host=REDIS_HOST,port=REDIS_PORT,password=REDIS_PASSWORD,db=0)
    # print(conn)
    # start = time.time()
