...
class CookieMiddleware(object):
    ...
    def __init__(self,settings):
        self.db = redis.StrictRedis(host=settings.get('REDIS_HOST'), port=settings.get('REDIS_PORT'),
    ...
    def process_request(self,request,spider):
        # cookies = demjson.decode(random.choice(self.db.hvals("cookies:weibo")))
        ...
        if 'm.weibo.cn' in request.url:
            ...
            cookies=demjson.decode(random.choice(self.db.hvals("user_cookies:weibo")))

        elif 'event.weibo.com' in request.url:
            # 抽奖微博的cookies
            cookies=demjson.decode(random.choice(self.db.hvals("winner_cookies:weibo")))
        #
        # else:
        #     print(request.url)
